...
/-! # Scalar multiplication on smooth vector bundles is smooth
In this file we prove that for every Cⁿ `𝕜`-vector bundle the action of `𝕜` on the total space is
Cⁿ, and as an application show that Cⁿ sections form a module not just over `𝕜` but over the Cⁿ
`𝕜`-valued functions.
-/
...
@[simps]
instance : SMul C^n⟮IB, B; 𝕜⟯ Cₛ^n⟮IB; F, E⟯ where
  smul f s := ⟨fun x ↦ f x • s x, f.contMDiff.smul s.contMDiff⟩

instance : IsScalarTower 𝕜 C^n⟮IB, B; 𝕜⟯ Cₛ^n⟮IB; F, E⟯ where
  smul_assoc a f s := by ext; simp [mul_smul]

instance : Module C^n⟮IB, B; 𝕜⟯ Cₛ^n⟮IB; F, E⟯ where
