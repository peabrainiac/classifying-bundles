...
This API is based mainly on `Mathlib.Geometry.Manifold.Algebra.Monoid` and
...
multiplication on vector spaces in favour of a `ContMDiffSMul` instance, so these can be renamed. -/
@[to_additive]
